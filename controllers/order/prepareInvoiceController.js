const {
  formatDate,
  getCurrentTimeFormatted,
} = require("../../middleware/getDateTime");
const { HOST } = require("../../config/index");
const axios = require("axios");
const PrepareInvoA = require("../../models/prepareinvoA");
const PrepareInvoB = require("../../models/prepareinvoB");

exports.index = async (req, res, next) => {};

exports.insertA = async (req, res, next) => {
  const items = req.body.items;
  for (let item of items) {
    let calWight = await axios({
      method: "post",
      url: `${HOST}master/calwight`,
      data: {
        itemNo: item.itemNo,
        qty: item.qty,
      },
    });
    let calCost = await axios({
      method: "post",
      url: `${HOST}master/calcost`,
      data: {
        itemNo: item.itemNo,
        qty: item.qty,
      },
    });
    console.log(calWight.data[0].grossWight);
    console.log(calCost.data[0].cost);
  }

  // console.log(calWight.data);
  // console.log(calCost.data);

  res.json("successs");

  // const insert = await PrepareInvoA.create({
  //   coNo: 410,
  //   OUDIVI: "OTT",
  //   OUFACI: "F10",
  //   orderNo: 1212100163,
  //   productNo: 6,
  //   OUOSSQ: 1,
  //   orderStartDate: 20211207,
  //   OUOSPE: 202112,
  //   customerNo: 100000001,
  //   customerChannel: 102,
  //   customerStartDate: 100000001,
  //   orderType: 21,
  //   payer: 100000001,
  //   OUCUCD: "THB",
  //   saleCode: 11000,
  //   OUCSCD: "TH ",
  //   OUFRE1: "YSEND",
  //   warehouse: 904,
  //   itemNo: 10010101014,
  //   OUITGR: "900 G   ",
  //   itemType: 100,
  //   OUITCL: "PK01 ",
  //   OUORST: 33,
  //   OUORQT: 2,
  //   OUORQA: 2,
  //   OUALUN: "PCS",
  //   OUCOFA: 1,
  //   OUDMCF: 1,
  //   OUSPUN: "PCS",
  //   OUORQS: 2,
  //   OUSTUN: "PCS",
  //   OUORQB: 2,
  //   grossWight: calWight.data[0].grossWight,
  //   netWight: calWight.data[0].netWight,
  //   OUDCCD: 2,
  //   OUSAPR: 70,
  //   OUGRPR: 140,
  //   OUSAAM: 140,
  //   OUPRMO: 8,
  //   OUDISY: "CREDIT 18 ",
  //   OUDWDT: formatDate(),
  //   OUCODT: formatDate(),
  //   OUUCOS: calCost.data[0].cost,
  //   OUUCCD: 1,
  //   OUUNMS: "PCS",
  //   OUORTK: 1,
  //   addressID: "INVTSP",
  //   OUINRC: 100000001,
  //   OURGDT: formatDate(),
  //   OURGTM: 501,
  //   OULMDT: formatDate(),
  //   OUCHNO: 2,
  //   OUCHID: "MVXSECOFR ",
  //   OULMTS: Date.now(),
  //   OUACOS: 73,
  //   OUTEPY: 30,
  //   OUDECU: 100000001,
  //   OURQWH: 904,
  // });
  // console.log(insert);
  // res.json(insert);
  // res.status(201).json(insert);
};

exports.insertB = async (req, res, next) => {
  try {
    await PrepareInvoB.create({
      coNo: 410,
      UCDIVI: "OTT",
      UCFACI: "F10",
      orderNo: 999999999,
      UCDLIX: 256144,
      productNo: 6,
      UCIVNO: 121024301,
      UCORDT: formatDate(),
      UCDWDT: formatDate(),
      UCCODT: formatDate(),
      UCDLDT: formatDate(),
      UCIVDT: formatDate(),
      UCYEA4: 2021,
      UCACDT: formatDate(),
      customerNo: 100000001,
      customerChannel: 102,
      UCCUST: 100000001,
      orderType: 21,
      payer: 100000001,
      UCCUCD: "THB",
      UCRAIN: 1,
      UCDMCU: 1,
      saleCode: 11000,
      UCCSCD: "TH ",
      UCFRE1: "YSEND",
      warehouse: 904,
      itemNo: 10010101014,
      UCITGR: "900 G   ",
      itemType: 100,
      UCITCL: "PK01 ",
      UCSTUN: "PCS",
      UCALUN: "PCS",
      UCSPUN: "PCS",
      UCPRMO: 8,
      UCDISY: "CREDIT 18 ",
      UCUCCD: 1,
      UCORTK: 1,
      addressID: "INVTSP",
      UCIVQT: 2,
      UCOFQS: 2,
      UCIVQA: 2,
      UCIVQS: 2,
      UCORQT: 2,
      UCORQS: 2,
      UCORQA: 2,
      UCORQB: 2,
      grossWight: 1,
      netWight: 1,
      UCSAAM: 140,
      UCSGAM: 163,
      UCCUAM: 140,
      UCUCOS: 73,
      UCDCOS: 73,
      UCDDF1: -1,
      UCDDF4: 1,
      UCTDEL: 1,
      UCTORL: 1,
      UCRQTY: 1,
      UCMPRO: 1,
      UCINRC: 100000001,
      UCROUT: "NKP007",
      UCRODN: 24,
      UCRGDT: formatDate(),
      UCRGTM: 3046,
      UCLMDT: formatDate(),
      UCCHNO: 4,
      UCCHID: "MVXSECOFR ",
      UCINPX: 2564,
      UCEXIN: 2564121024300,
      UCLMTS: 1638812003657,
      UCACOS: 73,
      UCTEPY: 30,
      UCDECU: 100000001,
      UCRQWH: 904,
    });
    res.status(201).json("Created");
  } catch (error) {
    next(error);
  }
};
